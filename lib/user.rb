class User < ActiveRecord::Base
  ...
  def get_recommendations_by_genre(genre_name)
    ...
    if self.age < 18
      recommendations = Anime.joins(:genre).where("genres.name = ? AND animes.age_rating IN ('G','PG') AND animes.id NOT IN (?)", genre_name, my_anime_ids).order('score DESC').limit(5)
    else
      recommendations = Anime.joins(:genre).where("genres.name = ? AND animes.id NOT IN (?)", genre_name, my_anime_ids).order('score DESC').limit(5)
    end
  ...
  def get_recommendations_by_my_ratings
    ...
    highest_genres.each {|genre| self.get_recommendations_by_genre(genre)}
  end
end
