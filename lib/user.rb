class User < ActiveRecord::Base
  ...
  def get_recommendations_by_genre(genre_name)
    if self.age < 18
      recommendations = Anime.joins(:genre).where("genres.name = ? AND animes.age_rating IN ('G','PG')", genre_name).order('score DESC').limit(5)
    else
      recommendations = Anime.joins(:genre).where("genres.name = ?", genre_name).order('score DESC').limit(5)
    end
